...
test('Check clicking option will change the sorting', async ({ page }) => {
  ...
  // create a copy of the names array and sort it in descending order
  const sortedNames = [...names].sort().reverse();
  // compare the names array with the sortedNames array
  expect(names).toEqual(sortedNames);
  console.log('Products are sorted in descending order by name');
});